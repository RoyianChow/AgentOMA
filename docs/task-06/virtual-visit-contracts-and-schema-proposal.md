...
| `appointmentRef` | Optional appointment reference | UUID | Yes | Task 04 synthetic contract; no production contract yet | Server | Not PHI (opaque) | Server-only | Read | Visit service | Unavailable in production until Task 04 is approved and integrated | Yes |
