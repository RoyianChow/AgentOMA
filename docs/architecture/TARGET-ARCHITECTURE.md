...
The evidence-based conclusion of this review is that **the current data platform and (once
confirmed) current hosting platform should be retained** — this is not a "no changes" report,
though. Several concrete, low-risk deltas from today's architecture are recommended, none of
which involve replacing the database or hosting vendor:
...
1. **Confirm and, if needed, enable Supabase PITR** — currently unconfirmed from repository
   ...
   requirement.
2. **Confirm the actual current hosting platform**, then explicitly pin function/compute
   execution to a Canadian region if it is not already pinned (Vercel's default is US-based;
...
4. **Decide, with privacy/legal input, whether a formal BAA-equivalent compliance product is
   needed** from the confirmed hosting vendor (concretely available today on Vercel at $350/mo if
   that is the platform).
5. **No change to the database schema, ORM, migration tooling, or auth provider** — Drizzle,
   file-based migrations, and better-auth all remain appropriate regardless of which managed
   Postgres or hosting platform is ultimately confirmed/selected.
...
flowchart TB
  ...
  subgraph host["Hosting platform — CONFIRM, then pin region\n(recommendation: retain current platform\nonce identified; if Vercel, pin to yul1/Montreal)"]
    appInstance["AgentOMA app instance(s)\n— unchanged code/framework"]
  end

  subgraph gh["🌐 GitHub / CI"]
    ci["Existing CI pipeline — unchanged\n+ NEW: consider adding a deploy-region\nassertion check (Workstream-4-derived idea,\nnot yet designed — see open decisions)"]
  end
  ...
  pg --> pitr
  ci -.->|"deploy — mechanism to be\nconfirmed with current platform owner"| host
...
do-nothing conclusion, but the changes are operational verification and one unexecuted runbook,
not new vendors.

## What this target explicitly does not include

- No new database vendor.
- No new hosting vendor (pending confirmation of the current one).
...
**Recommended application host:** retain the current platform once confirmed (most likely
Vercel, unconfirmed) — retain, with explicit Canadian region pinning. See ADR-002.

**Recommended deployment topology:** unchanged from today — single-region Canadian hosting,
single-region Canadian Supabase Postgres, pooled runtime connection / direct migration
connection, no multi-region or active-active topology recommended at current single-pharmacy
pilot scale.

**What should remain unchanged:** database vendor, ORM (Drizzle), migration tooling (file-based,
`db:generate`/`db:migrate`, `db:push` banned), auth provider (better-auth), route-group
architecture, the two-layer auth boundary (`proxy.ts` optimistic gate + `requirePortalUser()`
authoritative recheck), the sandbox isolation mechanism, and the CI job structure.

**What should change now (no migration, no cost, no new vendor):**
1. Confirm whether Supabase PITR is enabled; enable it if not.
2. Confirm the actual current hosting platform.
3. If that platform is Vercel (or becomes the confirmed choice), explicitly pin function
   execution region to Montréal (`yul1`).
4. Execute the backup/restore drill at least once and record the evidence, per the existing
   runbook.
...
concludes a database migration is warranted, the sequence would follow standard `pg_dump`/logical
replication to the new provider, replay of the existing file-based migration history to
reconstruct triggers/privileges/constraints (the same discipline already in use), a Task
11-reviewed cutover window, and a verified restore-drill against the new provider before
decommissioning the old one. No part of this sequence is scheduled or approved by this document.
...
- Confirmation of the actual current hosting platform (operational knowledge this review does
  not have access to).
- Whether a formal BAA-equivalent product is required from the hosting vendor (privacy/legal
  decision).
...
current official vendor documentation with a checked date. Lower confidence in two specific
areas, both disclosed at the point they occur rather than smoothed over: (1) the exact maximum
PITR window for Google Cloud SQL's enhanced backups could not be confirmed from the page fetched
during research; (2) Azure Container Apps' Canada East regional availability was inconsistently
documented and should be re-verified directly before being relied on. Neither gap affects the
"stay with current stack" recommendation, since both were only relevant to migration alternatives
